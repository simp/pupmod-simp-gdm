...
(`manifests/service.pp`, `spec/acceptance/suites/default/00_default_spec.rb`).
...
second run configures (`manifests/config.pp`).
...
- **`gdm` (`manifests/init.pp`)** — Public entry class (consumers
  `include 'gdm'`; **not** `assert_private()`'d). Calls
  `simplib::assert_metadata($module_name)` (`init.pp`), then `include`s the
  three component classes and pins ordering (`init.pp`):
  ...
  - `$dconf_hash` (`Dconf::SettingsHash`, **no default**) — supplied from module
    data (`data/common.yaml`); GDM `dconf` settings, deep-merged with the
    banner settings.
  - `$packages` (`Hash[String[1], Optional[Hash]]`, **no default**) — the package
    list, from module data (`data/common.yaml`, augmented per-OS in
    `data/os/*.yaml`). Setting this **overrides** (deep-merges) the default list.
  - `$settings` (`Gdm::CustomConf`, **no default**) — from module data
    (`data/common.yaml`); each entry becomes a `gdm::set` in `gdm::config`.
  ...
  - `$auditd` (`Boolean`) — defaults to
    ...
    (`init.pp`). **NOTE: this parameter is accepted but never referenced
  ...
  - `$pam` (`Boolean`) — defaults to
    ...
    (`init.pp`); gates the PAM local-access rule in `gdm::install`.
  - `$include_sec` (`Boolean`, default `true`) — documented as having **no
    effect** (`init.pp`); accepted for backward compatibility only.
  - `$banner` (`Boolean`, default `true`), `$simp_banner` (`String[1]`, default
    `'simp'`), `$banner_content` (`Optional[String[1]]`, default `undef`) —
    control the login-screen banner (`init.pp`).
  - `$display_mgr_user` (`String[1]`, default `'gdm'`) — the local user granted
    PAM access when `$pam` is true (`init.pp`).

- **`gdm::install` (`manifests/install.pp`)** — `assert_private()` +
  `simplib::assert_metadata` (`install.pp`). Installs `$gdm::packages` via
  `simplib::install` with `ensure => $gdm::package_ensure`
  (`install.pp`). Two optional integrations:
  - **PAM branch** (`install.pp`): when `$gdm::pam`, asserts the optional
    `simp/pam` dependency and declares `pam::access::rule` allowing
    `$gdm::display_mgr_user` LOCAL access (order 1).
  - **hidepid/systemd branch** (`install.pp`): only when `systemd` is in
...
- **`gdm::config` (`manifests/config.pp`)** — `assert_private()`
  (`config.pp`). Iterates `$gdm::settings` and emits a `gdm::set` per
  section/key/value (`config.pp`). Then, **guarded on the `gdm_version`
  fact** (`config.pp`):
  - `fail()` if GDM version is between `0.0.0` and `3` — unsupported
    (`config.pp`).
  - Otherwise declares `dconf::profile { 'gdm' }` with user/system/file entries
    (`config.pp`), builds `$_banner_settings` from the `$banner` /
    `$banner_content` / `simp_banners::fetch($simp_banner)` logic
    (`config.pp`), and declares `dconf::settings { 'GDM Dconf Settings' }`
    with `deep_merge($_banner_settings, $gdm::dconf_hash)` (`config.pp`).
  - If `gdm_version` is **not** set, emits a `notify` telling the operator an
    additional Puppet run is needed (`config.pp`) — the two-pass behavior.

- **`gdm::service` (`manifests/service.pp`)** — public class (no
  `assert_private()`). Declares `service { 'gdm': ensure => running, enable =>
  true }` (`service.pp`). If `$services` is set **and** `svckill` exists
  (`simplib::module_exist('svckill')`), it declares `svckill::ignore` for those
  services so `svckill` won't disable them (`service.pp`). `$services`
  defaults to `undef` in the manifest but is populated from module data
  (`data/common.yaml`).

- **`gdm::set` (`manifests/set.pp`)** — public defined type. Writes one
  `ini_setting` into `/etc/gdm/custom.conf` for the given `$section`
  (`Gdm::ConfSection`), `$key`, `$value` (`set.pp`), `require`ing
  `Class['gdm::install']` and `notify`ing `Class['gdm::service']`. It also
  `include 'gdm'` (`set.pp`).

Custom data types (`types/`):
- **`Gdm::ConfSection`** (`types/confsection.pp`) — `Enum` of the valid
  `custom.conf` section names (`daemon`, `security`, `xdmcp`, `gui`, `greeter`,
  `chooser`, `debug`, `servers`, `server-Standard`, `server-Terminal`,
  `server-Chooser`).
- **`Gdm::CustomConf`** (`types/customconf.pp`) — `Hash[Gdm::ConfSection,
...
- **Two-pass apply is required.** `gdm::config` only writes the `dconf`
  ...
  another run), the second run configures (`config.pp`; the
  acceptance spec runs the manifest twice, `00_default_spec.rb`).
...
- **`$auditd` is dead weight.** It is a parameter with a `simp_options::auditd`
  lookup default (`init.pp`) but is never consumed anywhere in the manifests
  — there is no auditd integration despite the docstring (`init.pp`).
- **`$include_sec` has no effect** (`init.pp`) — accepted for
  backward-compatibility only.
- **`$packages` / `$settings` / `$dconf_hash` deep-merge** across the Hiera
  hierarchy with `knockout_prefix: --` (`hiera.yaml`, `data/common.yaml`),
...
- **`svckill::ignore` is best-effort.** `gdm::service` only calls it when
  ...
  `service.pp`) — `svckill` is a fixture, not a declared dependency.
- **Optional integrations are guarded at runtime** with
  `simplib::assert_optional_dependency` and a fact/param check
  (`install.pp`) — `simp/pam` and `puppet/systemd` are declared as
...
- **GDM cannot handle `\n`** in banner text — `banner_content` must use the
  ...
  `cr_escape => true` (`init.pp`, `config.pp`).
...
| `init.pp` | `simp_options::auditd` | `false` (parameter is unused — see gotchas) |
| `init.pp` | `simp_options::pam` | `false` |
...
- Respect the two-pass model: configuration that depends on the `gdm_version`
  fact must tolerate the fact being absent on the first run (`config.pp`).
